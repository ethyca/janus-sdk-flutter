...
void main() {
  ...
  group('JanusLogger', () {
    ...
    test('logger handles all log levels correctly', () {
      ...
      expect(() => mockLogger.log('Verbose message', level: LogLevel.verbose), returnsNormally);
      expect(() => mockLogger.log('Debug message', level: LogLevel.debug), returnsNormally);
      expect(() => mockLogger.log('Info message', level: LogLevel.info), returnsNormally);
      expect(() => mockLogger.log('Warning message', level: LogLevel.warning), returnsNormally);
      expect(() => mockLogger.log('Error message', level: LogLevel.error), returnsNormally);
    ...
    test('logger handles metadata and errors', () {
      ...
      mockLogger.log('Info with metadata', level: LogLevel.info, metadata: {'key1': 'value1', 'key2': 'value2'});
      mockLogger.log('Error with exception', level: LogLevel.error, error: Exception('Test error'));
      mockLogger.log('Warning with both', level: LogLevel.warning, metadata: {'source': 'test'}, error: 'String error');
      mockLogger.log('Simple message', level: LogLevel.info);
      
      // Verify calls were logged
      expect(mockLogger.logCalls.length, 4);
      expect(mockLogger.logCalls[0].metadata, {'key1': 'value1', 'key2': 'value2'});
    ...
    test('_handleNativeLog safely handles unknown log levels', () async {
      ...
      final testCases = [
        {'message': 'Test with unknown level', 'level': 'unknown', 'metadata': null},
        {'message': 'Test with typo level', 'level': 'eror', 'metadata': null}, // typo in 'error'
        ...
        {'message': 'Test with mixed case', 'level': 'Error', 'metadata': null}, // Mixed case should work
      ];
      ...
      for (final testCase in testCases) {
        await TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.handlePlatformMessage(
          'janus_sdk_flutter',
          const StandardMethodCodec().encodeMethodCall(MethodCall('log', testCase)),
          (data) {},
        );
      }
      ...
      for (int i = 0; i < 5; i++) {
        expect(mockLogger.logCalls[i].level, LogLevel.info, reason: 'Unknown level should default to info');
      }
      
      // Mixed case 'Error' should be handled correctly (after toLowerCase())
      expect(mockLogger.logCalls[5].level, LogLevel.error, reason: 'Mixed case Error should work');
    });
    ...
    test('native log calls route through method channel to Flutter logger', () async {
      ...
      final testCases = [
        ...
        {'message': 'Native debug log', 'level': 'debug', 'metadata': {'source': 'native'}},
        {'message': 'Native info log', 'level': 'info', 'metadata': 'simple metadata'},
        {'message': 'Native warning log', 'level': 'warning', 'metadata': {'error': 'code', 'count': 123}},
        {'message': 'Native error log', 'level': 'error', 'metadata': ['item1', 'item2']},
        {'message': 'Unknown level log', 'level': 'unknown', 'metadata': null}, // Should default to info
      ];
      ...
      expect(mockLogger.logCalls[3].level, LogLevel.warning);
      expect(mockLogger.logCalls[3].metadata, {'error': 'code', 'count': '123'});
      ...
      expect(mockLogger.logCalls[5].level, LogLevel.info); // Unknown levels default to info
  ...
  group('DefaultJanusLogger', () {
    test('logs messages without throwing exceptions', () {
      ...
      expect(() => logger.log('Warning', level: LogLevel.warning), returnsNormally);
      expect(() => logger.log('Error', level: LogLevel.error, metadata: {'key': 'value'}), returnsNormally);
    });
  });
} 